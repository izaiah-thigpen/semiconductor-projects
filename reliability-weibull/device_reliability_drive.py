...
and extracts reliability metrics (characteristic life, MTTF, B10) and the hazard trend.
    ...
    python device_reliability_drive.py
...
# ---- paste the dataset's Google Drive share link (file shared "Anyone with the link") ----
DRIVE_LINK = "https://drive.google.com/file/d/1BUYG-u5pL8nElaZQkNgn3ZfjUDDvVFaU/view?usp=sharing"
...
def fetch_csv(link):
    """Download a CSV file from Google Drive into a pandas-readable buffer."""
    import requests
    fid = _file_id(link); s = requests.Session()
...
def load():
    df = pd.read_csv(fetch_csv(DRIVE_LINK)); df.columns = [c.strip() for c in df.columns]
    if "Failure_Type" not in df.columns or "Uptime (hrs)" not in df.columns:
        raise SystemExit("This file lacks 'Uptime (hrs)'/'Failure_Type' - DRIVE_LINK may point to the "
                         "wafer dataset. Put the IoT reliability CSV link here.\nColumns: " + ", ".join(df.columns))
